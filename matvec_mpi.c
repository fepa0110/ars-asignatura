...
*/
  if ( my_id == master )
  {
    a = ( double * ) malloc ( cant_rows * cant_cols * sizeof ( double ) );
    x = ( double * ) malloc ( cant_cols * sizeof ( double ) );
    b = ( double * ) malloc ( cant_rows * sizeof ( double ) );
    ...
    for ( i = 1; i <= cant_rows; i++ ) 
    {
      ...
      {
        a[k] = sqrt ( 2.0 / ( double ) ( cant_cols + 1 ) ) 
          * sin ( ( double ) ( i * j ) * pi / ( double ) ( cant_cols + 1 ) );
        k = k + 1;
      }
    }
/*
...
*/
    ...
    {
      // x[i] = 
       x[i] = sqrt ( 2.0 / ( double ) ( cant_cols + 1 ) ) 
        * sin ( ( double ) ( ( i + 1 ) * j_one ) * pi / ( double ) ( cant_cols + 1 ) );
    }
...
*/
  ...
  {
    a_row = ( double * ) malloc ( cant_cols * sizeof ( double ) );
    x = ( double * ) malloc ( cant_cols * sizeof ( double ) );
  }
/*
  Process 0 broadcasts the vector X to the other processes.
*/
  ierr = MPI_Bcast ( x, cant_cols, MPI_DOUBLE, master, MPI_COMM_WORLD );
...
*/
  {
    ...
    for ( i = 1; i <= num_procs-1; i++ )
    {
      dest = i;
      tag = num_rows;
      k = num_rows * cant_cols;

      ierr = MPI_Send ( a+k, cant_cols, MPI_DOUBLE, dest, tag, MPI_COMM_WORLD );
...
*/
  else
  {
    for ( ; ; )
    {
      ierr = MPI_Recv ( a_row, cant_cols, MPI_DOUBLE, master, MPI_ANY_TAG,
